            ...
            public void onPageFinished(WebView view, String url) {
                ...
                String jsCode = "javascript:(function() { " +
                        "setInterval(function() { " +
                            ...
                            "var killList = ['Contact Us', 'Download', 'PWTHOR owner', '@pwthor', '@pw_thor', 'Join Our Community', 'Telegram Community !!', 'Join Telegram Channel', 'Join Now!']; " +
                                ...
                                "if (element.children.length === 0 && element.innerText) { " +
                                    ...
                                    "for (var m = 0; m < killList.length; m++) { " +
                                        "if (txt === killList[m] || txt.includes(killList[m])) { " +
                                            ...
                                            // Aggressively target the highest level popup container if possible
                                            "var parentPopup = element.closest('div.fixed.inset-0, div.z-50, div[class*=\"overlay\"]');" +
                                            "if (parentPopup) { parentPopup.style.setProperty('display', 'none', 'important'); }" +
                                            "else if (box && box.tagName !== 'BODY' && box.tagName !== 'HTML') { " +
                            ...
                            // 5b. AGGRESSIVE POPUP CATCH-ALL (Look for common overlay classes)
                            "var overlays = document.querySelectorAll('div.fixed.inset-0.z-50, div[class*=\"overlay\"], div[class*=\"modal\"]'); " +
                            "for(var o = 0; o < overlays.length; o++) { " +
                                "if(!overlays[o].closest('.video-js, .plyr, video, [class*=\"player\"], [class*=\"vjs\"]')) { " +
                                     // Only hide if it contains specific text to avoid hiding generic menus
                                     "if(overlays[o].innerText && (overlays[o].innerText.includes('Join Now!') || overlays[o].innerText.includes('Telegram'))) { " +
                                        "overlays[o].style.setProperty('display', 'none', 'important'); " +
                                     "} " +
                                "} " +
                            "} " +
